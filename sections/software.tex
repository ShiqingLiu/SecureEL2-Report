\section{TEE Virtualization}

The TEE virtualization is to create a virtual TEE and
...
With the Armv9 architecture~\cite{armv9:2021} came out, Confidential Computing Architecture (CCA)~\cite{armCCA:2021} is introduced to the people at the first time. The aim of CCA is create a execution environment, Realm, who can be created from Normal world without inherent the trust from the creators. Note that CCA could create Realm to run a virtual machine, Realm virtual Machine (RMM), in it, and this Realm virtual Machine can be manage by Normal world hypervisor. Although Realm Virtual Machine is scheduled by the hypervisor or kernel, CCA does not need to trust them to gain the confidentiality, even the trusted code in Secure world. CCA could stablishing the trust of Realm through Attestation.

\begin{figure}
    \centering
    \includegraphics[scale=0.30]{CCA_architecture.png}
    \caption{CCA System Overview}
    \label{fig:CCA}
\end{figure}

CCA is different from TrustZone. The Trusted OS run on TrustZone is part of a chain of the turst. It depends on the high privilege firmware, such as SPM and Secure Monitor. There are two ways entering Trusted OS, either the yielding from Rich OS or the secure interrupt. Both may cause a world swithcing. However, the Realm Virtual Machine is managed by Normal world hypervisor. RMM just like normal VMs, being create and being allocated memory via the Host. Different from TOS, RMM may not be executed when the host is broken. In conlcusion, the software running in TrustZone is with confidentiality, intergity, and Authenticity. The software running in Realm is with confidentiality and intergity.

% graph of the CCA. 
% The disadvantage of TEEv is CCA compatibility
