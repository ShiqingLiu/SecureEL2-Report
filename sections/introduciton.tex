...
In Armv8.4-A, Secure EL2 is introduced as an optional feature. when the Secure EL2 enable, the platform firmware in EL3 can be move to EL1. So that the code in EL3 can be minimized. and use Secure Partion Manager to If EL2 in Secure world is supported, the processor needs use SCR\_EL3.EEL2 bit to enable it.  
...
However, TrustZone is not designed to be virtualizable in the past few years. All virtual machines managed by the same hypervisor shoudld trust the same TEE, while there are amount of vulnerabilities discovered in major venders' trusted kernel. 
